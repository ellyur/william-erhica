...
import logoImage from '@assets/image-removebg-preview (7)_1762319260739.png';
...
const Navigation = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 bg-background/95 backdrop-blur-md border-b border-primary/20 shadow-warm transition-all duration-300 ${
        ...
        <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center space-x-1">
              {navItems.map((item) => (
                <button
                  ...
                  className={`px-4 py-2 text-sm font-medium rounded-lg transition-all duration-300 hover:bg-primary/20 hover:text-primary ${
          ...
          <div className="md:hidden">
            <Button
              ...
              className="text-foreground/80 hover:bg-primary/20 hover:text-primary"
            >
          ...
          <div className="px-2 pt-2 pb-3 space-y-1 bg-background/95 backdrop-blur-md border-b border-primary/20">
            {navItems.map((item) => (
              <button
                key={item.name}
                onClick={() => scrollToSection(item.href)}
                className={`block w-full text-left px-3 py-2 text-base font-medium rounded-md transition-all duration-300 hover:bg-primary/20 hover:text-primary ${
                  activeSection === item.href.slice(1)
                    ? 'text-primary bg-primary/15'
                    : 'text-foreground/80'
                }`}
